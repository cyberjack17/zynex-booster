...
# --- SETTINGS & TRACKING ---
user_usage = {} 
TOTAL_LIKES_SENT = 0  # Global tracker
MAX_DAILY_LIMIT = 5 
VIP_USERS = [] # Put your ID here
...
def welcome(message):
    ...
    markup.add("⚡ Boost Now", "📊 Status", "❓ Help")
    bot.send_message(message.chat.id, "✅ **Zynex Ultra Pro: ONLINE**\n\nTap a button below to manage your boosts.", reply_markup=markup, parse_mode="Markdown")
...
def show_status(message):
    ...
    status_msg = (
        "📈 **Zynex Booster Stats**\n\n"
        f"👑 **VIP Status:** {'Active' if user_id in VIP_USERS else 'Inactive'}\n"
        f"🔥 **Total Likes Sent by Bot:** `{TOTAL_LIKES_SENT}`\n"
        f"📅 **Your Daily Usage:** `{usage}/{MAX_DAILY_LIMIT}`\n\n"
        "💡 *Status is reset when the server restarts.*"
    )
...
def handle_boost(message):
    ...
    if not search_token:
        bot.send_message(message.chat.id, "⚠️ Traffic still high. Try again in 1 minute.")
        return
    ...
    try:
        ...
        for _ in range(3):
            ...
            if t:
                ...
                success += 1
                TOTAL_LIKES_SENT += 1 # Increase total count
